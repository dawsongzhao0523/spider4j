...
# Spider Java - 跳过测试的发布脚本
# 直接构建和发布，跳过需要数据库的测试

set -e

echo "🚀 Spider Java Evaluation Framework - 跳过测试发布"
echo "================================================"
...
# 跳过测试，直接构建
echo "🔨 构建项目（跳过测试）..."
if mvn clean package -P release -DskipTests -q; then
    echo "✅ 构建成功"
else
    echo "❌ 构建失败"
    exit 1
...
if mvn deploy -P release -DskipTests -q; then
    echo ""
    echo "🎉 发布成功！"
    echo ""
    echo "📋 您的Spider Java Evaluation Framework已发布到:"
    echo "   GroupId:    com.chatdata"
    echo "   ArtifactId: spider-evaluation"
    echo "   Version:    1.0.0"
    echo ""
    echo "🔗 查看地址（发布后15-30分钟可用）:"
    echo "   Maven Central: https://search.maven.org/artifact/com.chatdata/spider-evaluation/1.0.0/jar"
    echo "   MVN Repository: https://mvnrepository.com/artifact/com.chatdata/spider-evaluation/1.0.0"
    echo "   Central Repo: https://repo1.maven.org/maven2/com/chatdata/spider-evaluation/1.0.0/"
    echo ""
    echo "📝 其他开发者可以这样使用您的库:"
    echo ""
    echo "   Maven依赖:"
    echo "   <dependency>"
    echo "       <groupId>com.chatdata</groupId>"
    echo "       <artifactId>spider-evaluation</artifactId>"
    echo "       <version>1.0.0</version>"
    echo "   </dependency>"
    echo ""
    echo "   Gradle依赖:"
    echo "   implementation 'com.chatdata:spider-evaluation:1.0.0'"
    echo ""
